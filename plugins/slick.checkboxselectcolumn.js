(function ($) {
  ...
  function CheckboxSelectColumn(options) {
    ...
    function handleClick(e, args) {
      ...
      if (_grid.getColumns()[args.cell].id === _options.columnId &&
          $(e.target).attr("class").indexOf('slick-select') > -1 &&
          $(e.target).attr("class").indexOf('locked') === -1) {
        // if editing, try to commit
        if (_grid.getEditorLock().isActive() && !_grid.getEditorLock().commitCurrentEdit()) {
          e.preventDefault();
          e.stopImmediatePropagation();
    ...
    function getColumnDefinition() {
      return {
        ...
        formatter: customBringgCheckboxSelectionFormatter
      };
    }
      ...
      if (dataContext) {
        if (!checkSelectableOverride(row, dataContext, grid)) {
          ...
          return _selectedRowsLookup[row]
            ? "<input id='selector" + UID + "' type='checkbox' checked='checked'><label class='pseudo-checkbox' for='selector" + UID + "'></label>"
            : "<input id='selector" + UID + "' type='checkbox'><label class='pseudo-checkbox' for='selector" + UID + "'></label>";
        }
      }
      return null;
    }
